...
public class DefaultResponseResult implements ResponseReactiveResult {
	...
	public Mono<Void> reactiveResponse(ServerWebExchange exchange,ServerHttpResponse response,Mono<DataBuffer> strDataBuffer,boolean isErrorResponse) {
		...
		if (isErrorResponse) {
			...
			return response.writeAndFlushWith(Flux.just(ByteBufFlux
					.just(response.bufferFactory().wrap(errorResultByte))));
		} else {
	...
	@Override
	public Mono<Void> reactiveResponseByte(ServerWebExchange exchange, ServerHttpResponse response, byte[] strDataBuffer,
		...
		if (isErrorResponse) {
			...
			response.setStatusCode(HttpStatus.OK);
			return  response.writeAndFlushWith(Flux.just(ByteBufFlux
					.just(response.bufferFactory().wrap(strDataBuffer))));
		}
	}
